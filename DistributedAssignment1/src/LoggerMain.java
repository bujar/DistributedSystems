            ...
            if (tm != null) {                
                if (tm.data.equals("showlogs")){
                	...
                	for (int j = 0; j<loggedMessages.size(); j++){
                		ArrayList<TimeStampedMessage> temp = concurrentMap.get(loggedMessages.get(j));
                		System.out.println(loggedMessages.get(j).stamp.value[0] + "  EVENT: " + loggedMessages.get(j).source +" "+ loggedMessages.get(j).kind + " " + loggedMessages.get(j).dest + " seq: " + loggedMessages.get(j).sequenceNumber + "\t\"" + loggedMessages.get(j).data + "\"");                    	
                		if (!temp.isEmpty()){
                	...
                	concurrentMap.put(tm, new ArrayList<TimeStampedMessage>());
                 	...
                 	for (TimeStampedMessage msg : loggedMessages){
                    	for (TimeStampedMessage entry : concurrentMap.keySet()){
                    		if (entry.stamp.concurrentWith(msg.stamp) && entry != msg)
